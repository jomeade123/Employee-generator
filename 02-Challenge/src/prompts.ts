...
export const rolePrompt = async (departments: { name: string, id: number }[]) => {
  return inquirer.prompt([
    {
      type: 'input',
      name: 'title',
...
export const employeePrompt = async (roles: { title: string, id: number }[], managers: { name: string, id: number }[]) => {
  return inquirer.prompt([
    {
      type: 'input',
      name: 'first_name',
